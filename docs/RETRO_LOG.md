...
| BL-032 | SMALL, 15–25 min | (folded into BL-029's total above) | **Neither wrong** — investigated, a real fix was tried, then correctly reverted on real evidence | JaCoCo 0.8.13 does fix the original instrumentation crash, but the same 3 real tests then failed/timed out at the TEST step instead (~20–50x slower, one genuine 240s timeout) — not shipped. `ACT-012` stays open with the real trade-off documented, not silently closed |
...
**Sprint totals:** 12 of 12 items done and proven, 0 abandoned. 1 item (`BL-032`) correctly concluded as a real, evidence-based non-fix rather than forced through. 2 items (`BL-029`, `BL-030+031`) required the parent session to take over mid-task after their implementing forks stalled — covered as the sprint's own significant implementation-mistake finding below.
...
3. A stray test-artifact file (`agent/claude_hook_test_spool_*.jsonl`) was found uncommitted twice across two different forks' worktrees — `agent/test_claude_code_hook.py` creates a uniquely-named real spool file per test run and does not appear to always clean it up. **Not fixed this sprint** (out of scope, low priority) — flagged as a real, minor test-hygiene gap for a future small item.
...
2. `ACT-012` (JaCoCo/JDK68) stays open with a real, evidence-based trade-off now documented instead of a silently-attempted fix — the Owner's real options (pin the isolated subprocess's own JDK, accept the gap, or invest further root-causing the JaCoCo/Mockito/JDK24 interaction) are laid out in both `docs/ACTION_QUEUE.json` and `docs/LESSONS.md`.
3. The Scrum-process research's own genuinely new finding (rework costs may not be visible in a same-day retro window) is itself a real, open methodological question this project's calibration process has no answer for yet — not actioned this sprint (it would require either waiting real calendar time before retro-ing an item, or building a mechanism to revisit an old item's verdict later), flagged here as a real candidate for the Owner to weigh, not decided unilaterally.

Retro prepared 2026-09-20, ~17:20–17:35 CEST, within the sprint the Owner explicitly authorized and started ("start sprint now"). All 12 items are merged into `master` and pushed to `origin/master`; the sprint's own real findings (3 duplicate-item discoveries, the agent-stalling pattern, the parent session's own merge omission, the honestly-reverted JaCoCo fix, and the newly-found `ACT-015` gap) are durably recorded here and cross-referenced in `docs/LESSONS.md`/`docs/ACTION_QUEUE.json` rather than left only in this retro. Two worktree directories (`agent-a32e298458203bf4f`, `agent-a79a4ba812d0da391`) remain on disk, un-removed, because their tracked agent processes still show as alive on the harness side despite their work being fully merged — flagged for manual cleanup rather than a forced process kill. Next sprint scoping awaits the Owner's discussion.

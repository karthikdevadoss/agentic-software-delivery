...
- **A framework-owned auto-configured `@Bean` type recurring as a silent-collision bug for the third time, never once logged here.** Real incident, third occurrence (2026-09-22, BL-039): api-gateway's BFF endpoint needed a plain (non-load-balanced) `RestClient.Builder` for calling a fixed-URL legacy stand-in, alongside the existing `@LoadBalanced` builder for service-to-service calls. Adding the plain one without an explicit `@Primary`/qualifier let `@ConditionalOnMissingBean`-based autoconfiguration hand the wrong builder to an existing consumer, breaking the gateway's own routing tests (caught by tests, not by review). CLAUDE.md already names this exact defect class ("a new `@Bean` of a framework-owned, auto-configured type is HIGH/CROSS_MODULE... `@ConditionalOnMissingBean` matches by TYPE, so an unqualified consumer elsewhere silently takes your bean") and this same project logged the identical mechanism once before (BL-007 item 7, a `@LoadBalanced` bean silently hijacking Eureka's own client) — but never here, in LESSONS.md, either time. **Rule:** any new `@Bean` method returning a framework-owned auto-configured type (`RestClient.Builder`, `RestTemplate`, `WebClient.Builder`, `ObjectMapper`, `TaskExecutor`, `SecurityFilterChain`, any `*Customizer`) gets an explicit `@Primary` or qualifier and a one-line comment naming which existing bean it must not collide with — checked mechanically where possible (see the new STATIC-tier check added the same day this lesson was written), not just caught by tests after the fact a third time.

- **A retro's own diagnosis can be wrong, in the direction that excuses the process, if nobody independently re-checks it.** Real incident (2026-09-23): Sprint 6's retro DRAFT claimed the sizing tool (`suggest_estimate()`) "had n=0 for every combination, so nothing could correct" a 0.01–0.11 estimate/actual ratio across all 7 items. An independent Shiva review found this false — 3 of 7 items had a real COMPUTED suggestion available, silently overridden with no recorded reason (a violation of `_calibration_process`'s own explicit rule), and in the one comparable historical case (BL-036) the override was empirically worse than the suggestion it replaced. The same review also caught the sprint's headline wall-clock figure quietly changing measurement basis (a sum of item spans reported as if it were the commit-to-commit figure prior sprints used) and a Sprint 7 backlog swap (BL-062 for BL-043) that was never flagged to the Owner. **Rule:** a retro that diagnoses "the tool/process failed" should be checked against the tool's own real recorded output before being trusted — a same-session self-diagnosis is exactly the kind of claim independent review exists to catch, not a formality to skip when the session feels confident in its own account.

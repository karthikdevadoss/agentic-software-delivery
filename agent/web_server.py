...
#
# Same shape as Scenarios A/B above (reused engine) -- only the target
# functions in agent/triage_execution.py differ.

async def triage_page_c(request: Request):
    return FileResponse(str(WEB_DIR / "triage-c.html"))


async def triage_reset_c(request: Request):
    try:
        return JSONResponse(await run_in_threadpool(triage_execution.reset_scenario_c))
    except triage_execution.TriageExecutionError as e:
        return JSONResponse({"error": str(e)}, status_code=502)


async def triage_reproduce_c(request: Request):
    try:
        return JSONResponse(await run_in_threadpool(triage_execution.reproduce_scenario_c))
    except triage_execution.TriageExecutionError as e:
        return JSONResponse({"error": str(e)}, status_code=502)
...
async def triage_diagnose_c(request: Request):
    body = await request.json()
    reproduction_result = body.get("reproduction_result") or {}
    result = await run_in_threadpool(triage_execution.diagnose_c, reproduction_result)
    return JSONResponse(result)


async def triage_reference_c(request: Request):
...
async def triage_generate_candidate_c(request: Request):
    body = await request.json()
    reproduction_result = body.get("reproduction_result") or {}
    generation = await run_in_threadpool(triage_execution.generate_candidate_patch_c, reproduction_result)
    if not generation.get("generated"):
        return JSONResponse({"generation": generation, "verification": None})
...
async def triage_verify_c(request: Request):
    return JSONResponse(await run_in_threadpool(triage_execution.verify_fix_c))


async def triage_approve_c(request: Request):
    body = await request.json()
    username = (body.get("username") or "").strip()
    password = body.get("password") or ""
    if not username or not password:
        return JSONResponse({"error": "admin username and password are required to approve"}, status_code=400)
    try:
        result = await run_in_threadpool(triage_execution.approve_scenario_c, username, password)
        return JSONResponse(result)
    except triage_execution.ApprovalAuthError as e:
        return JSONResponse({"error": str(e)}, status_code=401)
    except triage_execution.TriageExecutionError as e:
...
routes = [
    ...
    Route("/api/triage/scenario-c/reference", triage_reference_c, methods=["GET"]),

...
class EffortKwargTestCase(unittest.TestCase):
    def test_effort_is_passed_through_as_output_config_when_given(self):
        create_fn = mock.Mock(return_value=_FakeResponse("ok"))
        rg.call("NOVEL_IMPLEMENTATION_PROPOSAL", "s", "u", 100, create_fn=create_fn, effort="low")
        self.assertEqual(create_fn.call_args.kwargs["output_config"], {"effort": "low"})

    def test_no_effort_given_omits_output_config_entirely(self):
        create_fn = mock.Mock(return_value=_FakeResponse("ok"))
        rg.call("NOVEL_IMPLEMENTATION_PROPOSAL", "s", "u", 100, create_fn=create_fn)

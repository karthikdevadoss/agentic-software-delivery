...
/**
 * References the owning customer by {@code customerId} only -- billing-service
 * does not own the Customer entity (customer-service does, see
 * client/BillingCustomerClient), so no JPA relationship is declared and no
 * in-memory Plan -&gt; Customer navigation is possible here. Confirming a
 * customer actually exists is a real network call, not a join.
 */
@Entity

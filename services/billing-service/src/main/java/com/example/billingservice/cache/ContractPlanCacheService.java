 ...
 * comment in app/) for explicit composition where the exact behavior matters.
 *
 ...
 *
 * Ported unchanged from app/'s com.example.customer.cache.ContractPlanCacheService.
 */
@Component
public class ContractPlanCacheService {
    ...
    public ContractPlanResponse getActivePlan(Long customerId) {
        ...
        misses.increment();
        // Not wrapped in try/catch: a genuine "no active plan" 404 must
        // propagate exactly as it does for an uncached read -- caching must
        // never change API error behavior.

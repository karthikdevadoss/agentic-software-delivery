...
import com.example.customerservice.exception.ForbiddenWorkspaceAccessException;
 ...
 * bypasses this check but is still a real, separately-scoped,
 * server-verified JWT, never a bypass of authentication itself.
 *
 * A token with NO "role" claim at all -- the anonymous /auth/demo-token
 * issued by {@link DemoJwtIssuer#issueDemoToken()}, still used unchanged
 * by tests and internal automation -- is deliberately treated as
 * unscoped/legacy and bypasses this check. Only the persona login path
 * opts into workspace scoping.
 */
@Component
